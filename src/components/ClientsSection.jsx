import React from 'react';

const ClientsSection = () => {
  const clients = [
    {
      name: 'John Doe',
      feedback:
        "You're going to use a passage of Lorem Ipsum, you need to be sure there isn't anything embarrassing hidden in the middle of the text. All the Lorem Ipsum...",
    },
    {
      name: 'Jane Smith',
      feedback:
        'There are many variations of passages of Lorem Ipsum available, but the majority have suffered alteration in some form, by injected humour, or randomised words...',
    },
    {
      name: 'Alex Johnson',
      feedback:
        "You're going to use a passage of Lorem Ipsum, you need to be sure there isn't anything embarrassing hidden in the middle of the text. All the Lorem Ipsum...",
    },
  ];

  return (
    <section className="p-12 bg-gray-50 w-full">
      <h2 className="text-4xl font-bold text-gray-900 mb-4 text-center">Our Happy Clients</h2>
      <p className="text-gray-600 mb-12 text-center max-w-2xl mx-auto">
        We use only the best quality materials on the market to provide the best products to our patients.
      </p>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {clients.map((client, index) => (
          <div key={index} className="bg-white p-6 rounded-2xl shadow-md hover:shadow-lg transition-shadow">
            <div className="flex items-center mb-4">
              <img
                src={`https://i.pravatar.cc/100?img=${index + 30}`} // Random avatar
                alt={client.name}
                className="w-12 h-12 rounded-full mr-3"
              />
              <h3 className="text-lg font-semibold text-gray-900">{client.name}</h3>
            </div>
            <div className="flex text-yellow-400 mb-4">
              {[...Array(5)].map((_, starIndex) => (
                <svg
                  key={starIndex}
                  className="w-5 h-5 hover:scale-110 transition-transform cursor-pointer"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
            </div>
            <p className="text-gray-600 text-sm">{client.feedback}</p>
          </div>
        ))}
      </div>
    </section>
  );
};

export default ClientsSection;
